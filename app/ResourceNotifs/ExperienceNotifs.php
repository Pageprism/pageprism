...
class ExperienceNotifs extends RestfulNotifsAdapter implements RestfulResourceNotifs
{
    /**
     *  constructor.
    ...
    {
        $this->notifManager->broadcastInApp('new-experience', 'experiences', $resource->id, 'users', $resource->user_id);
    }

    public function notifyUpdate(Request $request, $resource)
    {
        $this->notifManager->broadcastInApp('update-experience', 'experiences', $resource->id, 'users', $resource->user_id);
    }

    public function notifyRemove(Request $request, $resource)
    {
        $this->notifManager->broadcastInApp('remove-experience', 'experiences', $resource->id, 'users', $resource->user_id);
    }
